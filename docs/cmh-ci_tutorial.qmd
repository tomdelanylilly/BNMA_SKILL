...
| **A terminal on the HPC cluster** | Posit Workbench (Positron, RStudio, or VS Code) or a direct SSH session — see "Where you run this" below |
...
Posit Workbench hosts three IDEs — **Positron**, **RStudio**, and **VS Code** — and Claude Code works the same way in all of them:

- **Terminal (main route)** — open the built-in terminal pane in whichever app you're using and run `claude` there, same as the commands below.
- **Extension** — some of these apps may have a Claude Code extension available, giving you a dedicated panel instead of a plain terminal. Check your app's extension/plugin marketplace first; fall back to the terminal if there isn't one installed.

Either way, once `claude` is running, everything from here on — starting a session, typing `/cmh-ci`, answering its questions — is identical.
