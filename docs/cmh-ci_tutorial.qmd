...
└── .claude/skills/cmh-ci/
    ...
    └── CLAUDE.md    ← project-level config (PRD write guard, execution style)
```
...
data. Alongside the plots, the skill writes a standalone runnable script
(`report.R`) and a narrative report (`report.Rmd`), rendering the latter
to `.html`.
...
set of outputs every run produces — the two plots plus `report.R` and the
`report.Rmd`/`.html` pair that records exactly how these numbers were
produced.


# Understanding the Outputs

## report.R — the standalone script

`report.R` is the actual reproduction mechanism — plain, sequential R
code with no chunk markers, runnable top to bottom with one `Rscript`
call. No RStudio, no chunk-by-chunk stepping required, though you can
still open it and read or step through it manually in any editor. It
covers everything the fit needed:

| Section | What it does |
|---|---|
| **Setup** | Load libraries, define paths (PRD hardcoded, QA resolved by pattern), config variables |
| **Data load** | Read PRD/QA, merge, filter to included studies |
| **Build BATMAN** | Construct JAGS matrices ($y$, $se$, $trt$, $n_a$, $ns$, $M$) |
| **Fit model** | Seeded inits, JAGS model definition, `jags.model()`, burn-in, `coda.samples()` |
| **Results** | Extract posteriors, build result table, fit pooled-placebo |
| **Forest plots** | Render both relative and absolute, save PNGs |
...
:::{.callout-tip}
The fit is seeded (see Methods' "Fit model" step), so re-running `report.R`
unmodified reproduces the exact same posterior draws and plots — not
just a statistically similar result. Edit paths, study lists, or model
settings first if you want a genuinely different run.
:::

## report.Rmd — the narrative report

`report.Rmd` has no code — the runnable form is `report.R` above. It's
what you open cold, months later, to answer "how was this produced, and
can I reproduce it": a short per-step workflow summary, the exact data
provenance (PRD/QA paths, any supplemental data merged in, `se_fallback`
and other manifest-recorded adjustments), which studies were included,
the design choices (`model_type`, filters), and links to the two output
plots plus a pointer to `report.R`.

## report.html

The `.html` is `report.Rmd` rendered — shareable by email or on
SharePoint, no R needed. Since `report.Rmd` has no code to run, this
renders in seconds.

All three files live in the programs folder:
...
Open the previous run's `report.R`, update file paths near the top if
they've changed, and run it (`Rscript report.R`). Or run `/cmh-ci`
fresh — it picks up the latest PRD and QA files automatically.

### "Switch from random-effects to fixed-effects"

At Step 6, choose **fixed-effect**. Or edit `model_type` (and the model
file it selects) in an existing `report.R` and re-run it.
...
### "Can I edit report.R and re-run?"

Yes — that's the point. Modify what you need (paths, study list, model
settings), then:
...
If you also changed something worth reflecting in the narrative (e.g. why
you re-ran it), edit `report.Rmd` and re-render its `.html`:
...
same deliverables: the forest plots themselves, a standalone `report.R`
that reproduces them exactly, and a `report.Rmd`/`.html` narrative pair
that records exactly which data and which decisions produced them — the
audit trail the earlier, hand-edited-script workflow never had.
