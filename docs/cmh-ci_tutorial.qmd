...
**Option A: Global install** (works from any folder, in any session) — place the skill at:
```
~/.claude/skills/cmh-ci/SKILL.md
```

**Option B: Project-level** (works for anyone whose session starts inside that folder):
```
/lillyce/qa/diabetes/bnma/obesity/data/shared/
├── .claude/skills/cmh-ci/SKILL.md
└── CLAUDE.md    ← optional 5-line pointer
```

A project-level install only applies when your session's working
directory is inside that specific folder — if you start sessions from
different places, use the global install (Option A) instead.

:::{.callout-warning}
Don't place a project-level copy under `/lillyce/prd/...`. Files created
there can get locked by a compliance/retention mechanism shortly after
creation — even the file's own owner can no longer edit or delete it
afterward. PRD is meant to be a curated, semi-annual read tier anyway;
QA is the live working copy this skill is designed to write into.
:::

The optional `CLAUDE.md` pointer lets users know the skill exists:
...
```bash
cd /lillyce/qa/diabetes/bnma/obesity/data/shared   # or your own working directory
