...
**What this skill does, in one breath:** it guides a statistician through
a Bayesian network meta-analysis (BNMA) of the Cardiometabolic Health
competitive-intelligence landscape, end to end. Concretely, it will
(a) show what's actually in the PRD/QA dataset — every study, compound,
phase, and evidence tier — before asking for a single decision;
(b) ask which studies to include, and surface any naming or route
conflicts for explicit sign-off; (c) fold in any new outside data — a
press release, a publication link, a hand-digitized slide — into the QA
workbook, row by row, with confirmation; and (d) fit the BNMA and deliver
the two forest plots (placebo-adjusted and absolute) plus a reproducible
per-run report. Nothing enters the model silently, and reviewing the data
without fitting anything is a complete, legitimate outcome on its own.
...
on every run, for every user; the point is a stable operator banner, not a
fresh description each session.
...
(placebo-adjusted and absolute), a comprehensive report, and a
...
- **Explicit yes** ("yes," "let's proceed," etc.) → proceed into Step 1.
- **Anything non-affirmative or ambiguous** → end the turn without touching
  anything. Nothing carries over, since nothing was touched.
...
- **No automated post-fit diagnostics are run** (no Rhat/ESS, no
  network-connectivity/consistency/DIC) — matches the production
  `EliLillyCo/CMH.BNMA` app. To verify a fit, inspect the posterior
...
Present **all studies from both sheets** (Observed and Prediction), each
as one numbered line in the identical format `N. <study_name> (<phase>) —
<compound>: <treatments>`, under an `OBSERVED (N studies)` /
`PREDICTION (M studies)` heading, then ask `Which studies do you want to
include?`. Never collapse the Prediction list into a compound-name summary
paragraph just because it's the second sheet — a Prediction study is
exactly as eligible as an Observed one, and hiding its phase/treatments
behind a name-only mention makes it too easy to pick the wrong study.
...
`/tmp/$(whoami)/cmh_ci_lib/study_selection_manifest.yaml` first — this is
where `run_bnma_pipeline.R` reads it from at fit time. Every study in the
data must appear with an explicit `include: true/false`;
`run_bnma_pipeline.R`'s missing-decision gate is unchanged and needs
exactly that. After a successful fit, Step 7 copies this manifest into the
programs folder as a kept deliverable, so the exact input that produced
the run is preserved alongside the script and report.

## Step 6 — Collect modelling preferences

Ask only the heterogeneity decision — no model-fitting preview:
...
Nothing else is asked here: region/route/evidence are not user choices
(`route_filter`/`evidence_filter` stay at their manifest default `"both"`),
and effect is not asked because Step 7 always produces both. Default
proceeds unless the user says otherwise; once confirmed, update the
manifest with `model_type`.
...
- **A copy of the exact `study_selection_manifest.yaml` used for the fit**
  → `/lillyce/qa/diabetes/bnma/obesity/programs/YYYYMMDD_<slug>/study_selection_manifest.yaml`
  (copy, don't move, from `/tmp/$(whoami)/cmh_ci_lib/` after the fit
  succeeds — the `/tmp` copy is scratch and may not survive the session).
  This is the exact input that produced the run — every study's explicit
  include/exclude decision and all design fields — kept alongside the
  script so the run is reproducible from the programs folder alone, not
  just describable from the report.

The programs folder holds **the script, the RMD report, and the
manifest** — the three things needed to reproduce and explain the run. It
does not hold `samples.rds`/`placebo_samples.rds`; those MCMC caches stay
in `/tmp` scratch (re-derivable by re-running the script against the kept
manifest). `run_bnma_pipeline.R`'s missing-decision gate still forces an
explicit `true`/`false` for every study — that behavior is unchanged; what
changed is that the manifest recording those decisions is now a kept
deliverable, and the report summarizes them in prose.

### RMD report

Generate a fresh, comprehensive `report.Rmd` every run (never accumulated
across runs), written to the same run's programs folder alongside the
script and manifest. It is the human-readable record of the run — thorough
enough that someone who wasn't in the session can understand exactly what
was fit, on what data, with what decisions, and why. Pull its content from
the manifest written in Step 5/6 rather than re-deriving anything. Cover:

- **Run header** — date, `<slug>`, endpoint (`effect_col`/`se_col`,
  effect direction), and the resolved PRD and QA file paths (with their
  dated filenames) the run read from.
- **Studies included** — the full list of `include: true` studies, each
  with its compound(s), phase, evidence tier (observed/prediction), and
  route. A table is fine here — this is the substance of the analysis, so
  enumerate it rather than compressing to a comma list.
- **Studies excluded** — the `include: false` studies with, where the
  session recorded one, the reason (out-of-scope population, naming/route
  conflict, superseded readout). This is the part of the audit trail that
  shows a study was *seen and ruled out*, not forgotten — worth stating
  explicitly in the comprehensive report even though the manifest also
  records it.
- **Design choices** — `model_type` (random vs. fixed effects),
  `route_filter`, `evidence_filter`, `region_filter`, and any other
  non-default manifest field, each with its resolved value (state the
  defaults too, e.g. filters at `"both"`, so the record is self-contained).
- **Supplemental / external data** — every row merged in via Step 2/3:
  source (the exact link/file), the observed-vs-predicted determination,
  which fields were extracted vs. derived, and any assumption surfaced to
  the user (derived SE and its method, per-arm `n` from a randomization
  ratio, etc.). If none was added, say so.
- **Any data adjustments applied at fit time** — SE fallback (and its
  recorded reason), placebo clamp (and reason), row exclusions, compound
  or treatment relabels — pulled from the corresponding manifest fields.
- **Reproduction** — the copied `run_bnma_pipeline.R` path, the copied
  manifest path, and each of the two plot invocations' exact arguments, so
  the run can be re-executed from the programs folder verbatim.
- **Results** — links to the two output plots (relative- and
  absolute-effect) in this run's output folder, and the headline posterior
  estimate per included treatment if readily available from the fit.

Keep it a `.Rmd` source file (not rendered) — comprehensive in content,
but still just text written in one pass, so producing it costs nothing
beyond the writing.
...
- No re-asking Step 6's `model_type` choice once made, and no post-hoc
  ...
  is used as-is
- No `samples.rds` or `placebo_samples.rds` in the programs folder — the
  MCMC caches stay in `/tmp` scratch (re-derivable by re-running the
  script against the kept manifest); the script, RMD report, and manifest
  are what persist there
...
present as a standalone file before 2026-08-27 — every place that
referenced it described the composition (A3's baseline + A2's delta)
without spelling out the resulting text; with the template, that
composition is now guaranteed by construction rather than hand-assembled.
...
(Appendix A) live inside `run_bnma_pipeline.R` itself — the four network
models are generated from one shared body by `make_network_model()`'s two
toggles rather than maintained as four near-identical strings, and the
pooled-placebo model is a standalone string — all passed to `jags.model()`
via `textConnection()`. There's no separate "materialize
`model_random.txt` to a file first" step; Appendix A is reference
documentation only, not something a step instructs you to write to disk.
...
read_sheet_with_fallback <- function(path, sheet_name, fallback_index, sheets = NULL) {
...
load_tier <- function(path, tier_label) {
  ...
  observed <- read_sheet_with_fallback(path, "Observed", fallback_index = 2, sheets = sheets)
  if (!is.null(observed)) {
    parts[["Observed"]] <- stringify_all(observed) %>%
      mutate(source_tier = tier_label, source_sheet = "observed", region = "global")
  }
  prediction <- read_sheet_with_fallback(path, "Prediction", fallback_index = 3, sheets = sheets)
